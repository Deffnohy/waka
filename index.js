...
var ab = await baper.resize(ukur1, ukur2).getBufferAsync(jimp.MIME_JPEG)
             resolve(ab)
                ...
                let buff = await getBuffer (`https://malesin.xyz/welcome2?username=${num.split("@")[0]}&groupname=${metadata.subject}&membercount=${dmember}&profile=${bup}&background=https%3A%2F%2Ftelegra.ph%2Ffile%2F99721d2ed3f5436f5bcbe.jpg`)
                var buffer = await getBuffer(ppuser)
he = `┌─❖\n│「 𝗛𝗶 👋 」\n└┬❖ 「 @${num.split("@")[0]}  」\n   │✑  𝗪𝗲𝗹𝗰𝗼𝗺𝗲 𝘁𝗼 \n   │✑  @subject\n   │✑  *Intro Dulu*\n   │✑  Nama : \n   │✑  Umur : \n   │✑  Askot : \n   └───────────────┈ ⳹`
let buttons = [{ buttonId: `halo`, buttonText: { displayText: `WELCOME` }, type: 1 }]
    let sis = {
 ...
 naze.sendMessage(anu.id, sis, {quoted: fgclink})
                ...
                let dmember = metadata.participants.length
                 let ppusr = await getBuffer(ppuser)
                      let imags = await getRandom('.jpg')
                        ...
                        let bup = await TelegraPh('./' + imags)
                        let participants = metadata.participants
                let buff = await getBuffer (`https://malesin.xyz/goodbye2?username=${num.split("@")[0]}&groupname=${metadata.subject}&membercount=${dmember}&profile=${bup}&background=https%3A%2F%2Ftelegra.ph%2Ffile%2F99721d2ed3f5436f5bcbe.jpg`)
                let buttons = [{ buttonId: `dahh`, buttonText: { displayText: `GOODBYE` }, type: 1 }]
    let sis = {
   document: fs.readFileSync('./media/doc/fake.pptx'),
  fileName : `GOODBYE`,
  caption: `Selamat tinggal @${num.split("@")[0]}\nSemoga tenang disanaa`,
  footer: titlebot,
  buttons: buttons,
  headerType: 4,
  contextInfo:{externalAdReply:{
